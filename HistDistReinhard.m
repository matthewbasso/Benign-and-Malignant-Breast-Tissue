function [HistDistR] = HistDistReinhard(P, TargetImage, n, verbose)

HistDistR = array2table(zeros(n,3),'VariableNames',{'HI_Hue','HI_Sat','HI_Val'});


for i = 1:n
    
    hsvImage = rgb2hsv(P(i).Image_Norm_Reinhard);
    hImage = hsvImage(:, :, 1);
    sImage = hsvImage(:, :, 2);
    vImage = hsvImage(:, :, 3);
...
%     hold off
    ...
    HistDistR.HI_Hue(i) = HistInter(hSource,hTarget);
    HistDistR.HI_Sat(i) = HistInter(sSource,sTarget);
    HistDistR.HI_Val(i) = HistInter(vSource,vTarget);
