...
% matrix
[DCh, H, E, Bg, M] = Deconvolve( Source, MSource);

% Vectorize to N x 3 matrix
DCh = reshape(DCh, [], 3);

% Find the 99th percentile of stain concentration (for each channel)
maxCSource = prctile(DCh, 99, 1);


%% MAIN NORMALIZATION STUFF
% 
DCh = bsxfun(@rdivide, DCh, maxCSource);
DCh = bsxfun(@times,   DCh, maxCTarget);


%% Reconstruct the RGB image 
Norm = Io*exp(DCh * -MTarget);
