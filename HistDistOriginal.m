function [HistDistO] = HistDistOriginal(P, TargetImage, n, verbose)

HistDistO = array2table(zeros(n,3),'VariableNames',{'HI_Hue','HI_Sat','HI_Val'});


for i = 1:n
    
    hsvImage = rgb2hsv(P(i).Image);
    hImage = hsvImage(:, :, 1);
    sImage = hsvImage(:, :, 2);
    vImage = hsvImage(:, :, 3);
...
%     hold off
    ...
    HistDistO.HI_Hue(i) = HistInter(hSource,hTarget);
    HistDistO.HI_Sat(i) = HistInter(sSource,sTarget);
    HistDistO.HI_Val(i) = HistInter(vSource,vTarget);
